...
Example:\n\
    ...
    parser.add_argument('-m', '--method', type=str, action='store', default=None, choices=['gsea','toppfun'],
                        help='Method for gene set analysis (gsea or toppfun)')
    parser.add_argument('-n', '--nhits', type=int, action='store', required=False, default=500,
                        help='Maximum number of top hits to extract, default = 500')
    parser.add_argument('-p', '--pvalue', type=float, action='store', required=False, default=0.05,
                        help='Maxium p-value threshold to export, default = 0.05')
    parser.add_argument('-q', '--qvalue', type=float, action='store', required=False, default=0.10,
                        help='Maximum FDR correction value to export, default = 0.10')
    parser.add_argument('-s', '--sheetname', type=str, action='store', default=None,
                        help='Sheetname if input file is Excel rather than text (required for *.xlsx)')
    parser.add_argument('-f', '--fformat', type=str, action='store', default="pipeliner",
    ...
    if keepLog:
        ...
        log.write("Exporting genes to {}, n={}, p={}, q={}, ".format(outfile.name, nhits, pvalue, qvalue))
        log.flush()
