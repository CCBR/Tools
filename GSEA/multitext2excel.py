...
from ncbr_huse import run_cmd, run_os_cmd, un_gzip, send_update, err_out, fasta_count, fasta_list
...
#                        help='Input file containing important things')
    parser.add_argument('-o', '--outfile', required=True, nargs='?', type=argparse.FileType('w'), default=None,
                        help='Output file for important results')
    parser.add_argument('-d', '--indir', required=False, type=str, action='store', default = '.',
...
#    infile = args.infile
    ...
    #
    send_update("multitext2excel.py successfully completed.  {} written.".format(outfile.name), log)
